...
impl DMA {
    ...
    pub fn write_interrupt(&mut self, value: u32) {
        // Update writable bits (0-23) if any are set in the write value
        // This allows clearing interrupt flags without changing configuration
        if (value & 0x00FF_FFFF) != 0 {
            self.interrupt = (self.interrupt & 0xFF00_0000) | (value & 0x00FF_FFFF);
        }

        // Handle write-1-to-clear for bits 24-30
